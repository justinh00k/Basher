...
const score_to_create_ratio = 30;
...
var banned_words = ["niger", "dick", "retard", "asshole", "shit", "shitty", "shitting", "fuck", "fucking", "fag", "faggot", "tranny", "bitch", "fucker", "cunt", "nigger", "nigga", "slut", "twat", "pussy", "cock", "boner", "motherfucker", "hitler", "penis", "clit", "dong", "kike", "pussies", "penises", "bitches", "semen"];
...
exports.mass_add_start = functions.https.onRequest(async(req, res) => {
    ...
    firestore.collection("Users").get().then((snapshot) => {
        ...
        snapshot.forEach((doc) => {
            ...
            //    firestore.collection("Users").doc(doc.id).set({ "stories_created": 0 }, { merge: true });
...
exports.userCreate = functions.auth.user().onCreate(async(new_data) => {
    ...
    if (!(new_data.displayName || false)) {
        ...
        new_data.displayName = (await_user.displayName || "Basher " + Math.floor(Math.random() * 999));
        new_data.displayName = new_data.displayName.substring(0, 15);
...
exports.userUpdate = functions.firestore.document('/Users/{userID}').onUpdate((change, context) => {
    ...
    if (data.logged_in || false) {
        ...
        if (new_time - (data.last_login || 0) > 1000 * 60 * 60 * 24) {
            ...
            firestore.collection('Users').doc(context.params.userID).update("score", admin.firestore.FieldValue.increment(points.top));

            firestore.collection('Messages').doc(context.params.userID).update({ "messages": admin.firestore.FieldValue.arrayUnion({ title: "Login bonus!", message: "+" + points.logged_in + " Points", timestamp: new_time }) });
        ...
        firestore.collection('Stories').doc(context.params.storyID).get().then((doc) => {
            ...
            if (votes === tweet_vote_threshold && score >= tweet_rating_threshold && (story_data.title || false)) {
                ...
                for (let i = 0; i < story_data.contributors; i++) {
                    ...
                    firestore.collection('Messages').doc(story_data.contributors[i]).update({ "messages": admin.firestore.FieldValue.arrayUnion({ title: "You-co-wrote a top story!", message: "+" + points.top + " Points: " + '<span onclick="get_user(\'' + story_data.id + '\')">' + story_data.title + '</span>', timestamp: new_time }) });
    ...
    function add_the_word() {
        ...
        firestore.collection('Stories').doc(context.params.storyID).get().then((doc) => {
            ...
            if (story_data) {
                ...
                firestore.collection('Messages').doc(story_data.pending_word.contributor).update({ "messages": admin.firestore.FieldValue.arrayUnion({ title: "Your word was approved!", message: "+" + points.added_word + " Points: " + story_data.pending_word.word, timestamp: new_time }) });
        ...
        firestore.collection('Stories').doc(context.params.storyID).get().then((doc) => {
            ...
            if (data.title_vote === 1 && data.submit_title) {
                ...
                if (newData.pending_title[adjusted_title_vote_position].votes >= neccesary_title_votes) {
                    ...
                    firestore.collection('Messages').doc(story_data.pending_title[adjusted_title_vote_position].contributor).update({ "messages": admin.firestore.FieldValue.arrayUnion({ title: "Your Title Was Chosen!", message: "+" + points.title_chosen + " Points: " + newData.title, timestamp: new_time }) });
...
common_words = [
    "hbo", "netflix", "hulu", "minecraft", "veggie", "shrek", "woah", "fortnite", "kaboom", "basher",
